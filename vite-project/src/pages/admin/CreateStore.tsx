...
import type { setFormDataT } from '../../../types/create-store.type';
...
export default function CreateStore(): React.ReactElement {
  const { createShop, isLoading, addCashierToShop } = useShopStore();
  const navigate = useNavigate();
  const { user, logout, isAuthenticated }: any = useAuthStore();
  ...
  const [formData, setFormData] = useState<setFormDataT>({
    name: '',
    type: 'retail',
    description: '',
    businessInfo: {
  ...
  const handleTeamMemberChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    const value = e.target.value;
    setTeamMemberInput(value);
    setShowTeamSuggestions(value.length > 0);
  };

  const addTeamMember = async (email: string, name: string = email) => {
    if (!email || !email.includes('@') || teamMembers.some((m: any) => m.email === email)) {
      return;
    }
  ...
  const handleTeamMemberKeyPress = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (name.includes('.')) {
      ...
      if (parent.includes('.')) {
        ...
        setFormData((prev: any) => ({
          ...prev,
          [grandParent as keyof setFormDataT]: {
            ...(prev[grandParent as keyof setFormDataT] as any),
            [parentProp as string]: {
              ...((prev[grandParent as keyof setFormDataT] as any)[parentProp] as any),
              [childProp]: value
            }
          }
        }));
        ...
        setFormData((prev: any) => ({
          ...prev,
          [parent as keyof setFormDataT]: {
            ...prev[parent as keyof setFormDataT],
            [child as string]: type === 'checkbox' ? checked : value
          }
        }));
      }
    } else {
      setFormData((prev: any) => ({
        ...prev,
        [name as keyof setFormDataT]: type === 'checkbox' ? checked : value
      }));
    }
  };
